...
export const blogPost = catchAsyncErrors(async (req, res, next) => {
  if (!req.files || Object.keys(req.files).length === 0) {
    return next(new ErrorHandler("Blog main Image is mandatry", 400));
  }

  const { mainImage, paraOneImage, paraTwoImage, paraThreeImage } = req.files;

  if (!mainImage) {
    return next(new ErrorHandler("Blog main image is maindatry", 400));
  }
  ...
  if (!title || !category || !intro) {
    return next(
      new ErrorHandler("Title, intro and catgory is require field", 400)
    );
  }
  ...
  if (
    ...
    return next(
      new ErrorHandler("Error occur while uploading one or more images", 500)
    );
  }
  const blogData = {
    title,
...
export const getSingleBlog = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    if (!blog.published) {
      return next(new ErrorHandler("Blog not published", 404));
    }
...
export const updateBlog = catchAsyncErrors(async (req, res, next) => {
  ...
  if (req.files) {
    ...
    if (paraTwoImage) {
      ...
      newBlogData.paraTwoImage = { // Corrected from paraOneImage to paraTwoImage
    ...
    if (paraThreeImage) {
      ...
      newBlogData.paraThreeImage = { // Corrected from paraOneImage to paraThreeImage
